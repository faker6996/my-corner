...
import { APP_ROLE, HTTP_METHOD_ENUM, LOCALE } from "@/lib/constants/enum";
 ...
 */
export async function callApi<T>(url: string, method: HTTP_METHOD_ENUM, data?: any, config?: AxiosRequestConfig & { silent?: boolean }): Promise<T> {
  try {
    ...
    if (axiosErr.response?.status === 401) {
      ...
      const currentLocale = [LOCALE.VI, LOCALE.EN].includes(pathSegments[0] as any) ? pathSegments[0] : LOCALE.VI;
