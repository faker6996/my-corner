...
export class BaseRepo {
  ...
  async insert<T>(data: Partial<T>, returning: string = "*"): Promise<T> {
    ...
    const jsonbColumns = (data.constructor as any).jsonbColumns || [];
    for (const [key, value] of Object.entries(data)) {
      if (value !== undefined) {
        columns.push(key);
        // Stringify JSONB if configured on model
  ...
  async update<T extends { id?: any }>(data: Partial<T>, returning: string = "*"): Promise<T | null> {
    ...
    // Get JSONB columns from model metadata
    const jsonbColumns = (data.constructor as any).jsonbColumns || [];
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      // Handle JSONB columns based on model metadata
      if (jsonbColumns.includes(key)) {
        values.push(JSON.stringify(value));
      } else {
        values.push(value);
   ...
   * Delete by id
   */
  async delete(modelClassOrTable: { table: string } | string, id: number | string): Promise<void> {
   ...
   */
  async count<T>(modelClass: { table: string }, conditions?: Partial<Record<keyof T & string, any>>): Promise<number> {
    const whereParts: string[] = [];
    const values: any[] = [];
    let idx = 1;
    if (conditions) {
   ...
   */
  async countWhere(modelClassOrTable: { table: string } | string, whereSQL: string, params: any[] = []): Promise<number> {
    const table = typeof modelClassOrTable === "string" ? modelClassOrTable : modelClassOrTable.table;
   ...
   */
  async updateWhereIn(modelClassOrTable: { table: string } | string, set: Record<string, any>, key: string, valuesIn: any[]): Promise<number> {
   ...
   */
  async sum<T>(modelClass: { table: string }, column: keyof T & string, conditions?: Partial<Record<keyof T & string, any>>): Promise<number> {
    const whereParts: string[] = [];
    const values: any[] = [];
    let idx = 1;
    if (conditions) {
  ...
  async avg<T>(modelClass: { table: string }, column: keyof T & string, conditions?: Partial<Record<keyof T & string, any>>): Promise<number> {
    const whereParts: string[] = [];
    const values: any[] = [];
    let idx = 1;
    if (conditions) {
