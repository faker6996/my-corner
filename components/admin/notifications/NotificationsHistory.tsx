...
import { DataTable, DataTableColumn, DataTableQuery, Badge, Button } from "@underverse-ui/underverse";
...
export default function NotificationsHistory({ refreshTrigger }: NotificationsHistoryProps) {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        const sp = new URLSearchParams({ page: '1', pageSize: '1000' }).toString();
        const res = await fetch(`${API_ROUTES.USERS.LIST}?${sp}`);
  ...
  const loadNotifications = async (newQuery = query) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({
        page: newQuery.page.toString(),
        pageSize: (newQuery.pageSize * 10).toString(), // Load more for grouping
        ...(newQuery.sort && {
          sortBy: newQuery.sort.key,
          sortOrder: newQuery.sort.order
        })
      });

      const res = await fetch(`/api/notifications/sent?${params}`);
      const json = await res.json();
      const notifications: NotificationRecord[] = json?.data?.data || [];
